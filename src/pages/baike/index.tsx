...
/**
 * 规范化 anchorId，支持多种格式的 ID
 * - 如果包含斜杠，提取最后一部分（如 'system/updates' -> 'updates'）
 * - 清理前缀的连字符和特殊字符
 * - 转换为小写
 */
function normalizeAnchorId(anchorId: string): string {
  ...
  // 如果包含斜杠，提取最后一部分
  let cleanId = anchorId
  if (cleanId.includes('/')) {
    cleanId = cleanId.split('/').pop() || cleanId
  }
  
  // 清理前缀的连字符和特殊字符，转换为小写
  cleanId = cleanId.replace(/^-+/, '').replace(/^️-/, '').toLowerCase().trim()
 ...
 * 根据 anchorId 获取对应的组件
 */
export function getBaikeComponent(anchorId: string): React.ReactNode | null {
  if (!anchorId) return null
  
  const cleanId = normalizeAnchorId(anchorId)
  const Component = componentMap[cleanId]
 ...
 * 检查是否有对应的 TSX 组件
 */
export function hasBaikeComponent(anchorId: string): boolean {
  if (!anchorId) return false
  
  const cleanId = normalizeAnchorId(anchorId)
  return cleanId in componentMap
}
