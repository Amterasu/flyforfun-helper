...
import { getSection } from "../../../utils/configData";
 ...
 */
export function getContentItemsByAnchorId(
  anchorId: string,
  ...
  for (const [sectionKey, section] of Object.entries(sectionMap)) {
    ...
    for (const [subsectionKey, subsection] of Object.entries(
      ...
      const subsectionTitleZh =
        (subsection as any).translatedTitle ||
        (subsection as any).title_zh ||
        (subsection as any).title_cn ||
        "";
      const subsectionIdEn = titleToAnchorId(subsectionTitleEn);
      const subsectionIdZh = subsectionTitleZh
        ? titleToAnchorId(subsectionTitleZh)
        : "";
      ...
      if (subsectionIdEn === cleanId || subsectionIdZh === cleanId) {
        ...
        if (
          ...
          Array.isArray((subsection as any).content)
        ) {
        ...
        if ((subsection as any).subsections) {
          for (const [subSubsectionKey, subSubsection] of Object.entries(
            (subsection as any).subsections
          )) {
            if (
              (subSubsection as any).content &&
              Array.isArray((subSubsection as any).content)
            ) {
              ...
              const normalizedKey = subSubsectionKey
                .toLowerCase()
                .replace(/\s+/g, "-")
                .replace(/[^a-z0-9-]/g, "");
              let subSubsectionTitle =
                titleMap?.[normalizedKey] ||
      ...
      if ((subsection as any).subsections) {
        for (const [subSubsectionKey, subSubsection] of Object.entries(
          ...
          const subSubsectionTitleEn =
            (subSubsection as any).title || subSubsectionKey;
          const subSubsectionTitleZh =
            (subSubsection as any).translatedTitle ||
            (subSubsection as any).title_zh ||
            (subSubsection as any).title_cn ||
            "";
          const subSubsectionIdEn = titleToAnchorId(subSubsectionTitleEn);
          const subSubsectionIdZh = subSubsectionTitleZh
            ? titleToAnchorId(subSubsectionTitleZh)
            : "";

          if (
            (subSubsectionIdEn === cleanId || subSubsectionIdZh === cleanId) &&
            (subSubsection as any).content &&
            (subSubsection as any).content.length > 0
          ) {
    ...
    for (const [subsectionKey, subsection] of Object.entries(
      ...
      if ((subsection as any).subsections) {
        for (const [subSubsectionKey, subSubsection] of Object.entries(
          ...
          if (
            (subSubsectionIdEn.includes(cleanId) ||
              cleanId.includes(subSubsectionIdEn) ||
              subSubsectionIdZh.includes(cleanId) ||
              cleanId.includes(subSubsectionIdZh)) &&
            (subSubsection as any).content &&
            (subSubsection as any).content.length > 0
          ) {
            if (!bestMatch) {
      ...
      // 包含匹配的一级子章节
      if (
        subsectionIdEn.includes(cleanId) ||
        cleanId.includes(subsectionIdEn) ||
        subsectionIdZh.includes(cleanId) ||
        cleanId.includes(subsectionIdZh)
      ) {
        if (!bestMatch) {
          const result: ContentItem[] = [];
